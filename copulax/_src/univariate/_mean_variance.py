...
def mean_variance_ldmle_params(
    stats: dict, gamma: float, sample_mean: float, sample_variance: float
) -> tuple[float, float]:
    """Derive mu and sigma from sample moments using the mean-variance mixture identities."""
    # obtaining mu and sigma estimates
    mu: float = sample_mean - stats["mean"] * gamma
    sigma_sq: float = (sample_variance - stats["variance"] * lax.pow(gamma, 2)) / stats[
        "mean"
    ]

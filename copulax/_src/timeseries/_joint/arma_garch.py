...
class ArmaGarch(TimeSeriesModel):
    ...
    def __init__(
        ...
        *,
        ...
        if (not isinstance(mean_order, tuple)) or len(mean_order) != 2:
            raise ValueError(f"mean_order must be a (p, q) tuple; got {mean_order!r}.")
        if (not isinstance(var_order, tuple)) or len(var_order) != 2:
            raise ValueError(f"var_order must be a (p, q) tuple; got {var_order!r}.")
    ...
    def _build_cold_start(
        ...
        if self.p > 0:
            ...
            eps_proxy = jnp.asarray(y, dtype=float).reshape(-1) - jnp.mean(
    ...
    def _separable_warm_start(
        ...
        return {
            "phi": cast("dict", arma_fit.params)["phi"],
            "theta": cast("dict", arma_fit.params)["theta"],
            "mu": cast("dict", arma_fit.params)["mu"],
    ...
    def _make_objective(
        self,
        wrapper: StandardisedResidual,
    ) -> Callable[[Array, Array, Array, Array, tuple], Array]:
        def objective(
            raw: Array,
            y: Array,
            init_y_lags: Array,
    ...
    def fit(
        ...
        *,
        ...
        n_var = sum(
            jnp.atleast_1d(jnp.asarray(v, dtype=float)).size for v in var_dict.values()
        )
        n_params_total = self.p + self.q + 1 + int(n_var) + wrapper.n_shape_params
        ...
        fitted_residual_dist = wrapper.to_distribution(
            residual,
            name=f"{cast('Univariate', self.residual_dist).name}-stdresid",
        )
        ...
        if name is None:
            name = (
                ...
                f"-{cast('Univariate', self.residual_dist).name}"
            )
        return type(self)(
    ...
    def _recursion_inputs(
        ...
        state = self._build_initial_state(
            ...
            mode=init,
            backcast_length=backcast_length,
            residual_params=cast("dict", self.residual_params),
        )
        return y_arr, state
    ...
    def stats(self) -> dict:
        ...
        mean_is_stat = ar_is_stationary(cast("Array", self.phi))
        mean_is_inv = (
        ...
        var_helper_fitted = self.var_model(
            ...
            q=self.q_var,
            residual_dist=self.residual_dist,
            **dict(cast("dict", self.var_params)),
    ...
    def summary(
        ...
        *,
        ...
        if y is None:
            ...
            ll = float(
                self.loglikelihood(y, init=init, backcast_length=backcast_length)
            )
        ...
        mean_section = ParamSection(
            ...
            rows=iter_param_rows(
                {k: cast("dict", self.params)[k] for k in ("phi", "theta", "mu")},
        ...
        res_section = residual_section(
            ...
            se["residual"],
            dist_name=display_residual_name(rd.name),
        )
        header = (
    ...
    def _deserialise(
        ...
        if "params_schema" in metadata:
            ...
            kwargs["var_params"] = {k: params[k] for k in var_keys if k in params}

...
def _ppf(dist, uniform_data, cubic):
    ...
    params: dict = dist.example_params()
    support = dist.support(params)
    s: str = " with cubic spline approximation" if cubic else ""; breakpoint()
    ...
    # testing jit works
    jitted_ppf = jit(dist.ppf, static_argnames='cubic')(data, params, cubic=cubic)

    # testing gradients
    gradients(dist.ppf, f"{dist.name} ppf", data, params)  #- TODO this seems to be the slow part. maybe re-use jitted ppf
...
#     assert all(v.ndim == 0 and v.shape == 0 and v.size == 1 for v in stats.values()), f"{dist} stats values are not scalars"

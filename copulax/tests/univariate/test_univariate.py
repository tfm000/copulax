...
)  # note has to be a tuple otherwise pytest parametrize will remove elements as it scans along, causing test skipping
...
IVTF_RVS_COMBINATIONS = tuple(
    (dist, size) for dist in IVTF_DISTRIBUTIONS for size in rvs_sizes
)
...
def test_support(dist):
    ...
    assert support.ndim == 1 and support.shape == (
        2,
    ), f"{dist} support is not a flattened array"
...
def test_cdf(dist, continuous_data, discrete_data):
    ...
    assert np.all(0 <= output) and np.all(
        output <= 1
    ), f"cdf not in [0, 1] range for {dist}"
...
def _ppf(dist, uniform_data, cubic):
    ...
    assert np.all(output >= support[0]) and np.all(
        output <= support[1]
    ), f"ppf not in support range for {dist}{s}."
...
def test_stats(dist):
    ...
    assert all(
        isinstance(k, str) for k in stats.keys()
    ), f"{dist} stats keys are not strings"
    assert all(
        isinstance(v, Scalar) for v in stats.values()
    ), f"{dist} stats values are not scalars"
    assert all(
        v.ndim == 0 and v.shape == () and v.size == 1 for v in stats.values()
    ), f"{dist} stats values are not scalars"

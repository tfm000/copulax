...
PPF_SCIPY_CONFIGS = [
    ...
    (gh, {"lamb": 1.5, "chi": 2.0, "psi": 1.0, "mu": 0.5, "sigma": 1.0, "gamma": 0.3}),
...
class TestPPFAgainstScipy:
    ...
    def test_ppf_matches_scipy_brent(self, dist, params):
        ...
        cx_ppf = np.array(
            dist.ppf(q=jnp.array(q), params=params, brent=True, maxiter=50)
        ).flatten()
        sp_ppf = sp.ppf(q)
    ...
    def test_ppf_matches_scipy_cubic(self, dist, params):
        ...
        q = np.linspace(0.05, 0.95, 20)
        cx_ppf = np.array(
            dist.ppf(q=jnp.array(q), params=params, brent=False, nodes=500, maxiter=50)
        ).flatten()
        sp_ppf = sp.ppf(q)
...
class TestPPFInverseConsistency:
    ...
    def test_cdf_ppf_roundtrip_brent(self, dist, params):
        ...
        assert_inverse_consistency(
            dist, params, rtol=1e-6, n_points=20, maxiter=50, brent=True
        )
...
class TestPPFBoundary:
    ...
    def test_ppf_at_zero_and_one(self, dist, params):
        ...
        if np.isfinite(lo):
            np.testing.assert_allclose(
                ppf_0, lo, atol=1e-3, err_msg=f"{dist.name} PPF(0) != lower bound"
            )
        else:
        ...
        if np.isfinite(hi):
            np.testing.assert_allclose(
                ppf_1, hi, atol=1e-3, err_msg=f"{dist.name} PPF(1) != upper bound"
            )
        else:
...
class TestPPFCubicVsDirect:
    ...
    def test_cubic_vs_direct(self, dist, params):
        ...
        direct = np.array(
            dist.ppf(q=q, params=params, brent=True, maxiter=50)
        ).flatten()
        cubic = np.array(
            dist.ppf(q=q, params=params, brent=False, nodes=200, maxiter=50)
        ).flatten()
...
class TestPPFEdgeCases:
    ...
    def test_nan_q_returns_nan(self):
        ...
        x = np.array(
            normal.ppf(q=q, params={"mu": 0.0, "sigma": 1.0}, maxiter=50)
        ).flatten()
    ...
    def test_single_q_cubic_fallback(self):
        ...
        cubic = float(
            normal.ppf(
                q=q_single, params={"mu": 0.0, "sigma": 1.0}, brent=False, maxiter=50
            ).flatten()[0]
        )
        direct = float(
            normal.ppf(
                q=q_single, params={"mu": 0.0, "sigma": 1.0}, brent=True, maxiter=50
            ).flatten()[0]
        )

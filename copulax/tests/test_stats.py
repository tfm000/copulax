...
class TestSkew:
    ...
    def test_matches_scipy(self, bias):
        ...
        for data in [
            ...
            np.testing.assert_allclose(
                cx, sp, atol=1e-8, err_msg=f"Skew mismatch (bias={bias})"
            )
    ...
    def test_known_positive_skew(self):
        ...
        rng = np.random.RandomState(0)
        data = rng.exponential(1, 50000)
        s = float(skew(jnp.array(data)))
        np.testing.assert_allclose(
            s, 2.0, rtol=0.15, err_msg="Exponential skewness should be ~2"
        )
...
class TestKurtosis:
    ...
    def test_matches_scipy(self, fisher, bias):
        ...
        for data in [
            ...
            np.testing.assert_allclose(
                ...
                sp,
                atol=1e-8,
                err_msg=f"Kurtosis mismatch (fisher={fisher}, bias={bias})",
            )
    ...
    def test_normal_excess_near_zero(self):
        ...
        np.testing.assert_allclose(
            k, 0.0, atol=0.1, err_msg="Normal excess kurtosis should be ~0"
        )
    ...
    def test_normal_pearson_near_three(self):
        ...
        np.testing.assert_allclose(
            k, 3.0, atol=0.1, err_msg="Normal Pearson kurtosis should be ~3"
        )

    def test_uniform_excess_kurtosis(self):
        """Uniform distribution excess kurtosis = -6/5."""
        rng = np.random.RandomState(0)
        data = rng.uniform(0, 1, 100000)
        k = float(kurtosis(jnp.array(data), fisher=True))
        np.testing.assert_allclose(
            k, -6 / 5, atol=0.05, err_msg="Uniform excess kurtosis should be -6/5"
        )

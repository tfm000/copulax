...
t_params={'nu': 3.0, 'mu': 0.01, 'sigma': 2.5}
g_params = {'alpha': 2.0, 'beta': 1.0}
n_params = {'mu': -1.0, 'sigma': 2.5}
vars = normal.rvs(100, n_params)
# print(student_t.cdf(vars, t_params))
# func = lambda x, p: student_t.cdf(x, p).sum()
# func2 = lambda x, p: student_t.pdf(x, p).sum()
# print(grad(func, argnums=[0, 1])(vars, t_params))
# jit(normal.rvs, static_argnums=0)(100, n_params)
# rvs=gh.rvs(size=1000)
uvars = random.uniform(DEFAULT_RANDOM_KEY, 100)
eps = 1e-5
uvars = jnp.where(uvars > eps, uvars, eps)
uvars = jnp.where(uvars < 1-eps, uvars, 1-eps)
ppf = student_t.ppf(uvars, t_params)
func3 = lambda q, p: student_t.ppf(q, p).sum()
ppf_grad=grad(func3, argnums=[0, 1])(uvars, t_params)
print(ppf_grad)
# breakpoint()
